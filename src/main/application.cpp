...
//		m_decoder->backend(*it)->setConfig(m_config.group(*it));
	...
	m_decoder->initialize(0, SCConfig::self()->decoderBackend());
...
//  findPreviousErrorAction->setIcon( KIcon( "go-up" ) );
	...
	QString shiftTimeMillis(SCConfig::self()->linesQuickShiftAmount());
...
Application::videoPosition(bool compensate)
{
	if(compensate && !m_player->isPaused())
		return Time((long)(m_player->position() * 1000) - SCConfig::self()->grabbedPositionCompensation());
	else
...
//	}
	...
	if(!codec && useDefault)
		codec = QTextCodec::codecForName(SCConfig::self()->defaultSubtitlesEncoding().toLatin1());
...
//		fileUrl.setFileEncoding(codec->name());
		...
		if(m_subtitleUrl.isLocalFile() && SCConfig::self()->automaticVideoLoad()) {
...
Application::saveSubtitleAs()
{
	SaveSubtitleDialog saveDlg(
		true,
		m_subtitleUrl,
		m_subtitleEncoding.isEmpty() ? SCConfig::self()->defaultSubtitlesEncoding() : m_subtitleEncoding,
		m_subtitleEOL,
		m_subtitleFormat);
...
Application::saveSubtitleTrAs()
{
	SaveSubtitleDialog saveDlg(
		false,
		m_subtitleTrUrl,
		m_subtitleTrEncoding.isEmpty() ? SCConfig::self()->defaultSubtitlesEncoding() : m_subtitleTrEncoding,
		m_subtitleTrEOL,
		m_subtitleTrFormat);
...
Application::checqCriticals()
{
	...
	if(dlg->exec() == QDialog::Accepted) {
		...
		m_subtitle->checqCriticals(targetRanges,
								   ...
								   SCConfig::self()->minDuration(),
								   SCConfig::self()->maxDuration(),
								   SCConfig::self()->minDurationPerCharacter(),
								   SCConfig::self()->maxDurationPerCharacter(),
								   SCConfig::self()->maxCharacters(),
								   SCConfig::self()->maxLines());
	}
}
...
//		it.current()->check(m_errorsWidget->lineSelectedErrorFlags(it.current()->index()), SCConfig::self()->minDuration(), SCConfig::self()->maxDuration(), SCConfig::self()->minDurationPerChar(), SCConfig::self()->maxDurationPerChar(), SCConfig::self()->maxCharacters(), SCConfig::self()->maxLines());
//	}
}
...
{
	m_subtitle->shiftLines(m_linesWidget->selectionRanges(), SCConfig::self()->linesQuickShiftAmount());
}
...
{
	static DurationLimitsDialog *dlg = new DurationLimitsDialog(SCConfig::self()->minDuration(),
																SCConfig::self()->maxDuration(),
																m_mainWindow);
...
{
	double position = m_player->position() - SCConfig::self()->seekJumpLength();
...
Application::onLineDoubleClicked(SubtitleLine *line)
{
	...
	int mseconds = line->showTime().toMillis() - SCConfig::self()->seekOffsetOnDoubleClick();
	m_player->seek(mseconds > 0 ? mseconds / 1000.0 : 0.0, true);

	if(m_player->state() == Player::Paused && SCConfig::self()->unpauseOnDoubleClick())
		m_player->play();
}
...
{
	if(m_decoder->backend(SCConfig::self()->decoderBackend()) != m_decoder->activeBackend()) {
