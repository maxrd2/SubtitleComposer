...
Scripting::RangeList::RangeList(const SubtitleComposer::RangeList & backend, QObject * parent):QObject(parent), m_backend(backend)
{
}

QObject *Scripting::RangeList::complement() const {
	return new Scripting::RangeList(m_backend.complement(), const_cast < Scripting::RangeList * >(this));
} QObject *Scripting::RangeList::range(int rangeIndex) const {
...
} bool Scripting::RangeList::contains(int index) const {
	return m_backend.contains(index);
} bool Scripting::RangeList::isEmpty() const {
	return m_backend.isEmpty();
} int Scripting::RangeList::rangesCount() const {
	return m_backend.rangesCount();
} int Scripting::RangeList::indexesCount() const {
	return m_backend.indexesCount();
} int Scripting::RangeList::firstIndex() const {
	return m_backend.firstIndex();
} int Scripting::RangeList::lastIndex() const {
	return m_backend.lastIndex();
} void Scripting::RangeList::clear()
{
	m_backend.clear();
}

void Scripting::RangeList::trimToIndex(int index)
{
	m_backend.trimToIndex(index);
}

void Scripting::RangeList::trimToRange(const QObject * object)
{
