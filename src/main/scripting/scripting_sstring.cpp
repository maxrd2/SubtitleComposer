...
Scripting::SString::SString(const SubtitleComposer::SString & backend, QObject * parent):QObject(parent), m_backend(backend)
{
}

bool Scripting::SString::isEmpty() const {
	...
	return m_backend.length();
} QString Scripting::SString::plainText() const {
	return m_backend.string();
} void Scripting::SString::setPlainText(const QString & string, int styleFlags)
{
	m_backend.setString(string, styleFlags);
}

QString Scripting::SString::richText() const {
	return m_backend.richString();
} void Scripting::SString::setRichText(const QString & string)
{
	m_backend.setRichString(string);
}

const QChar Scripting::SString::charAt(int index)
{
...
void Scripting::SString::setCharAt(int index, const QChar & chr)
{
	if(index >= 0 && index < m_backend.count())
		m_backend[index] = chr;
}

int Scripting::SString::styleFlagsAt(int index) const {
	if(index < 0 || index >= m_backend.count())
		return 0;
	return m_backend.styleFlagsAt(index);
} void Scripting::SString::setStyleFlagsAt(int index, int styleFlags) const {
	if(index >= 0 && index < m_backend.count())
		m_backend.setStyleFlagsAt(index, styleFlags);
} int Scripting::SString::cummulativeStyleFlags() const {
	return m_backend.cummulativeStyleFlags();
} bool Scripting::SString::hasStyleFlags(int styleFlags) const {
	return m_backend.hasStyleFlags(styleFlags);
} void Scripting::SString::setStyleFlags(int index, int len, int styleFlags)
{
	if(index < 0 || index >= m_backend.count())
		return;
...
void Scripting::SString::setStyleFlags(int index, int len, int styleFlags, bool on)
{
	if(index < 0 || index >= m_backend.count())
		return;
...
void Scripting::SString::clear()
{
	m_backend.clear();
}

void Scripting::SString::truncate(int size)
{
	if(size >= 0)
...
QObject *Scripting::SString::remove(int index, int len)
{
	if(index >= 0 && index < m_backend.count())
...
QObject *Scripting::SString::removeAll(const QString & str, bool regExp, bool caseSensitive)
{
	if(regExp)
...
} QObject *Scripting::SString::right(int len) const {
	return new SString(m_backend.right(len), const_cast < Scripting::SString * >(this));
} QObject *Scripting::SString::mid(int index, int len) const {
	if(index < 0 || index >= m_backend.count())
