...
ZoomBuffer::zoomedBuffer(quint32 timeStart, quint32 timeEnd, WaveZoomData **buffers, quint32 *bufLen)
{
	*bufLen = 0;
	...
	m_reqStart = qMin(timeStart * m_waveBuffer->sampleRateMillis() / m_samplesPerPixel, m_waveformZoomedSize);
	m_reqEnd = qMin(timeEnd * m_waveBuffer->sampleRateMillis() / m_samplesPerPixel, m_waveformZoomedSize);
	m_reqLen = bufLen;
