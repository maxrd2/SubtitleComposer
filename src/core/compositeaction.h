...
namespace SubtitleComposer {
	...
	explicit CompositeAction(const QString & description, bool immediateExecution = true, bool delaySignals = true):
		Action(description), m_actions(), m_immediateExecution(immediateExecution), m_delaySignals(delaySignals) {
		...
		/// returns the contained action when there's only one (returns 0 otherwise)
