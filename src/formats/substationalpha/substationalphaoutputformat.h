...
namespace SubtitleComposer {
	...
	protected:
		...
		virtual QString dumpSubtitles(const Subtitle & subtitle, bool primary)const {
			...
			QString ret = builder.arg(formatData ? formatData->value("ScriptInfo") : m_defaultScriptInfo)
			...
			for(SubtitleIterator it(subtitle); it.current(); ++it) {
				...
				QString showTimeArg = builder.sprintf("%01d:%02d:%02d.%02d",
													showTime.hours(),
													showTime.minutes(),
													showTime.seconds(),
				...
				QString hideTimeArg = builder.sprintf("%01d:%02d:%02d.%02d",
													hideTime.hours(),
													hideTime.minutes(),
													hideTime.seconds(),
		...
		SubStationAlphaOutputFormat():OutputFormat("SubStation Alpha", QStringList("ssa")), m_defaultScriptInfo(s_defaultScriptInfo), m_defaultStyles(s_defaultStyles), m_events(s_events), m_dialogueBuilder(s_dialogueBuilder) {
		}
	...
	SubStationAlphaOutputFormat(const QString & name, const QStringList & extensions, const QString & scriptInfo, const QString & styles, const QString & events, const QString & dialogueBuilder):
		OutputFormat(name, extensions), m_defaultScriptInfo(scriptInfo), m_defaultStyles(styles), m_events(events), m_dialogueBuilder(dialogueBuilder) {
		}
	...
	protected:
		...
		AdvancedSubStationAlphaOutputFormat():SubStationAlphaOutputFormat("Advanced SubStation Alpha", QStringList("ass"), s_defaultScriptInfo, s_defaultStyles, s_events, s_dialogueBuilder) {
		} static const char *s_defaultScriptInfo;
		static const char *s_defaultStyles;
		static const char *s_events;
		static const char *s_dialogueBuilder;
