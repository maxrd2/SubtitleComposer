...
GstCaps* GStreamer::audioCapsFromFormat( const WaveFormat& format, bool addSampleRate )
{
	GstCaps* caps = gst_caps_new_simple(
		format.isInteger() ? "audio/x-raw-int" : "audio/x-raw-float",
...
static QString state( GstState state )
{
	switch ( state )
	{
		case GST_STATE_VOID_PENDING:	return "STATE_VOID_PENDING";
		case GST_STATE_NULL:			return "STATE_NULL";
		case GST_STATE_READY:			return "STATE_READY";
		case GST_STATE_PAUSED:			return "STATE_PAUSED";
		case GST_STATE_PLAYING:			return "STATE_PLAYING";
